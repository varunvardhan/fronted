...
const Dashboard = () => {
  ...
  const handleReset = () => {
    ...
    // Clear results last to prevent flickering
    setAllResumeResults([]);
    
    toast.success("Analysis stopped and all data has been reset", {
      position: "top-center",
      autoClose: 2000,
    });
  };
...
*/
  ...
  const validateForm = () => {
    const desc = typeof jobDescription === 'string' ? jobDescription.trim() : '';
  ...
  const handleAnalyze = async () => {
    ...
    // console.log("Current jobDescription:", jobDescription);
  ...
  <div className="space-y-2">
    <div className="flex items-center justify-between">
      <span className="font-bold text-lg">Attach Resume</span>
      <span className="text-xs text-gray-500">Only PDF/DOC/DOCX Format</span>
    </div>
    <div className="relative">
      ...
      <div className="flex items-center justify-between p-3 text-sm border border-gray-300 rounded-lg bg-gray-50">
        ...
        <button className="px-4 py-2 bg-blue-600 text-white text-xs rounded-md hover:bg-blue-700">
          Browse
        </button>
      </div>
    </div>
  ...
  {/* CV Scoring Notes */}
  <div className="space-y-2">
    <span className="font-bold text-lg">CV Scoring Notes</span>
    <span className="text-xs text-gray-500">  ( Keywords for scoring )</span>
    <textarea
      ...
      value={cv_notes}
      onChange={(e) => setCvNotes(e.value)}
    />
  </div>

  {/* Additional Notes */}
  <div className="space-y-2">
    <span className="font-bold text-lg">Additional Notes</span>
    <span className="text-xs text-gray-500">  ( Optional comments )</span>
    <textarea
      className="w-full h-28 p-3 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
...
{allResumeResults.length > 0 && (
  <div className="mt-3">
    <h3 className="font-bold text-lg text-gray-800 px-2 mb-1">Results ({allResumeResults.length})</h3>
        ...
        return (
          <li
            ...
            className={`p-3 rounded-lg transition-all flex items-center gap-3 ${
              isProcessing
                ? "bg-gray-4100 cursor-not-allowed"
                : "bg-white hover:bg-gray-200 cursor-pointer border border-gray-100"
            }`}
          ...
          >
            ...
            <div className="flex-1 min-w-0 space-y-1">
              <div className="flex justify-between items-center">
                <p className="font-medium text-gray-900 truncate pr-2">{result.name}</p>
                <span className={`text-xs ${
                  isProcessing ? 'text-gray-500' : 
                  isProcessed ? 'text-green-500' : 'text-gray-400'
                }`}>
